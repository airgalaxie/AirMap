...
DynMap.prototype = {
	...
	initialize: function() {
		...
		function getWorldDisplayTitle(world) {
			...
			if(vanillaTitles[world.name])
				return vanillaTitles[world.name];

			var path = world.name.substring(world.name.indexOf(':') + 1);
		...
		$.each(me.worlds, function(index, world) {
			...
			world.element = $('<li/>')
				...
				.attr({ title: world.name })
				.text(getWorldDisplayTitle(world))
				.append(maplist = $('<ul/>')
	...
	update: function() {
		...
		$.getJSON(me.formatUrl('update', { world: me.world.storageid, timestamp: me.lasttimestamp, reqid: me.reqid }), function(update) {
	...
	getTileUrl: function(tileName) {
		...
		if(tile == null) {
			...
			tile = this.registeredTiles[tileName] = url + escape(me.world.tileid + '/' + tileName);
		}
		return tile;
	},
