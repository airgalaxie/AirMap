...
componentconstructors['markers'] = function(dynmap, configuration) {
	...
	function loadmarkers(world) {
		removeAllMarkers();
		var worldEntry = dynmap.worlds[world];
		var storageId = worldEntry ? worldEntry.storageid : world;
		var url = concatURL(dynmap.options.url.markers, '_markers_/marker_' + storageId + '.json');
