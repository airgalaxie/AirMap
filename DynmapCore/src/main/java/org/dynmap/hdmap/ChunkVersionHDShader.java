...
public class ChunkVersionHDShader implements HDShader {
    ...
    private final ArrayList<Integer> unknown_vers;
    	
    public ChunkVersionHDShader(DynmapCore core, ConfigurationNode configuration) {
        this.unknown_vers = new ArrayList<>();
    ...
    private class OurShaderState implements HDShaderState {
        ...
        public boolean processBlock(HDPerspectiveState ps) {
            ...
            for (DataVersionMap versionmap1 : versionmap) {
                if (ver <= versionmap1.dataVersion) {
                    c.setColor(versionmap1.defcolor);
                    match = true;
                    break;
                }
            }
    ...
    public void addClientConfiguration(JsonObject mapObject) {
        mapObject.addProperty("shader", name);
    }
    @Override
