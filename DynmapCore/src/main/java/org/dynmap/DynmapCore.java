...
public class DynmapCore implements DynmapCommonAPI {
    ...
    private String dynmapCompatibilityVersion = "unknown";
    private String airMapVersion = "unknown";
    private String airMapBuild = "local";
    private String airMapRevision = "unknown";
    ...
    public boolean enableCore(EnableCoreCallbacks cb) {
        ...
        /* Print product and compatibility version info */
        Log.info("AirMap " + airMapVersion);
        Log.info("Build " + airMapBuild);
        Log.info("Revision " + airMapRevision);
        Log.info("Dynmap compatibility API " + dynmapCompatibilityVersion);
        Log.info("Implementation versions: plugin=" + plugin_ver + ", core=" + version);
    ...
    public boolean processCommand(DynmapCommandSender sender, String cmd, String commandLabel, String[] args) {
        ...
        if (args.length > 0) {
            ...
            if (c.equals("render") && checkPlayerPermission(sender,"render")) {
                ...
                sender.sendMessage("AirMap " + airMapVersion);
                sender.sendMessage("Build " + airMapBuild);
                sender.sendMessage("Revision " + airMapRevision);
                sender.sendMessage("Dynmap compatibility API " + dynmapCompatibilityVersion);
    ...
    private void loadVersion() {
        ...
        if(val != null) {
            ...
            dynmapCompatibilityVersion = (String)val.getOrDefault("dynmap-compatibility-version", "unknown");
            airMapVersion = (String)val.getOrDefault("airmap-version", "unknown");
            airMapBuild = (String)val.getOrDefault("airmap-build", "local");
            airMapRevision = (String)val.getOrDefault("airmap-revision", "unknown");
        }
    }
