...
public abstract class DynmapWorld {
     ...
     * derived solely from the canonical world identity. */
    ...
    /** Common-core legacy aliases plus adapter-supplied platform names. */
    ...
    /** Historical lossy mapping, retained only for migration and alias lookup. */
    public static String legacyWorldName(String n) {
        return (n != null)?n.replace('/', '-').replace('[', '_').replace(']', '_'):null;
    }
    /** Filesystem-safe, deterministic ID. Never use this as the public world ID. */
