...
public class FabricWorld extends DynmapWorld {
    ...
    /**
     * Return the reversible, filesystem- and URL-safe storage name for a
     * canonical Minecraft world ID. Registry identifiers cannot contain '@'
     * or '+', so they are unambiguous readable separators for ':' and '/'.
     */
    public static String getStorageName(String canonicalName) {
    ...
    private static List<String> getLegacyLevelNames(String name, boolean nether, boolean theEnd) {
        ...
        if (Level.OVERWORLD.identifier().toString().equals(canonicalName)) {
            ...
            if (separator > 0 && separator < canonicalName.length() - 1) {
                aliases.add(canonicalName.substring(0, separator) + "_" + canonicalName.substring(separator + 1));
            }
        }
        aliases.removeIf(canonicalName::equals);
        return Collections.unmodifiableList(aliases);
    }
    ...
    public FabricWorld(DynmapPlugin plugin, Level w) {
        ...
        storageName = getStorageName(getLevelName(w));
        setLevelLoaded(w);
    }
    ...
    public void setLevelLoaded(Level w) {
        world = w;
        storageName = getStorageName(getName());
        legacyNames = getLegacyLevelNames(w);
